...
const handleBorrow = async (row) => {
  ...
  try {
    await ElMessageBox.confirm(
      `Are you sure you want to borrow "${row.title}"?`,
      "Borrow Confirmation",
      {
    ...
    const res = await borrowBook({ bookId: row.bookId });
    if (res.code === 200) {
      ...
      >
        ...
        >
          <template #default="{ row }">
            <el-link type="primary" @click="viewBookDetail(row)">
              {{ row.title }}
            </el-link>
          </template>
        </el-table-column>
        ...
        <el-table-column label="Actions" width="100" fixed="right">
          <template #default="{ row }">
            <el-button
              type="success"
              size="small"
              :disabled="!row.stock || row.stock <= 0 || row.status !== 'AVAILABLE'"
